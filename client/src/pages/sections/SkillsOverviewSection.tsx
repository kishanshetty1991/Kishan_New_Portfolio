...
export const SkillsOverviewSection = (): JSX.Element => {
  return (
    <section id="skills" className="flex flex-col w-full items-start gap-9 relative">
      <header className="flex flex-col max-w-[924px] items-start gap-3 relative">
        <div className="flex flex-col items-start gap-1.5 relative">
          <p className="relative [font-family:'Roboto_Mono',Helvetica] font-bold text-[#ffffff99] text-base tracking-[0] leading-[normal]">
            &lt;Discover&gt;
          </p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 w-full">
        {skillCategories.map((category, index) => (
          <Card
            key={index}
            className="rounded-[10px] overflow-hidden border border-solid border-[#4c8eff33] bg-transparent"
          >
            <CardContent className="flex flex-col items-start gap-[19px] p-[27px]">
              <div className="flex items-center gap-3.5 w-full">
                ...
                <h3 className="flex-1 [font-family:'Poppins',Helvetica] font-semibold text-[#ffffffcc] text-lg tracking-[0] leading-[normal]">
                  {category.title}
                </h3>
              </div>
              ...
              <div className="flex flex-wrap items-start gap-[9px_12px]">
                {category.skills.map((skill, skillIndex) => (
                  <Badge
                    key={skillIndex}
                    variant="outline"
                    className="inline-flex items-center justify-center gap-2.5 px-2.5 py-1.5 rounded-[20px] border border-solid border-[#ffffff33] bg-transparent hover:bg-transparent"
                  >
                    <span className="font-normal text-white [font-family:'Poppins',Helvetica] text-sm tracking-[0] leading-[normal]">
                      {skill}
                    </span>
                  </Badge>
                ))}
